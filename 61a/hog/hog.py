...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # 다이스의 합을 구하는 것을 돕기 위해 만듬
    sum = 0 

    # From the hint, the dice's result should be come out, even if sow_sad rule are hit.
    # 힌트에서 sow_sad 룰에 맞더라도, 다이스의 값은 무조건 나와야 한다고 함
    sow_sad = False

    # while loop 
    # 다이스의 횟수 만큼 while loop
    while num_rolls > 0:
        # store value
        # 주사위값 저장
        value = dice()
        
        #check the sow_sad condition
        if value == 1:
            sow_sad = True
        sum += value
        num_rolls -= 1
...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    # Not using list and for loop
    # 제곱의 수에서 십의 자리로나눈 숫자들중 작은 숫자의 자리수가 k 이고 return k + 3 
    k = score ** 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # 각 턴 마다 어떻게 적용하는지 활용하는 방법

    # num_rolls 0이면, piggy_points 전략을 써야 하기 때문에, 바로 piggy_points 함수에 넣어준다
    if num_rolls == 0:
        return piggy_points(opponent_score)
    # 그게 아닐경우 일반적인 roll_dice 함수에 넣어준다.
    else:
...
def more_boar(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # 특이한 경우의 수만 나열할 경우,
    # 맨 앞자리가 0일경우, int 형은 0을 생략하기 때문에,
    # 문자형으로 변환후, 맨 앞자리를 0으로 채워 넣어준다.
    # 그 후에 다시 첫번째 자리와 남은 수를 비교한다.
    # 그리고 자리수가 세자리인 경우에는, 
    # 똑같이 zfill을 사용하여 채워줘도 아무 변화가 없기에 무방비,
    # 그리고 세자리수 비교는 맨 앞자리수 비교후 남은 자리 수 비교한다.
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # 점수가 goal 보다 작을경우에 계속해서 while loop
    ...
    def say(score0, score1):
        ...
        if score > last_score:
            ...
            if running > running_high:
                ...
                print('Player', who, 'has reached a new maximum point gain.', running_temp, 'point(s)!')
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_index = 1
    for i in range(1, 11):
        aver_roll_dice = make_averaged(roll_dice, trials_count)
        score = aver_roll_dice(i, dice)
        if score > max:
            max_index = i
            max = score
    return max_index
